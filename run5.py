...
def execute(bob: Bob):
    ...
    yield from bob.foreward(distance=285, speed=200)
    yield from bob.turn(degree=-60, speed=75, then=Stop.BRAKE)
    # Marketwares solved
    yield from bob.arc(radius= 1700,angle=15,speed=100)
    ...
    yield from bob.turn(degree=-38, speed=75, then=Stop.HOLD)
    wait(500)
